const confirmKeywords = async function (e) {
  ...
  let parentNodesToCheck = getParentNodesWithAtLeastTwoChilds(
    filterest.hiddenElements
  );
  ...
  while (
    filterest.suggestedElements.length === 0 &&
    !parentNodesToCheck.find((p) => p.tagName === "BODY")
  ) {
...
const getParentNodesWithAtLeastTwoChilds = function (elements) {
  ...
  elements.forEach((element) => {
    if (
      !parentNodesToCheck.find(
        (elem) =>
          filterest.getSelector(element.parentNode) ===
          filterest.getSelector(elem)
      )
    ) {
  ...
  while (searchParents) {
    let elementsWithOneChild = parentNodesToCheck.filter(
      (element) => element.children.length <= 1
    );
    ...
    if (elementsWithOneChild.length > 0) {
      elementsWithOneChild.forEach((e) => {
        let index = parentNodesToCheck.findIndex(
          (element) =>
            filterest.getSelector(element) === filterest.getSelector(e)
        );
        ...
        if (
          !parentNodesToCheck.find(
            (elem) =>
              filterest.getSelector(e.parentNode) ===
              filterest.getSelector(elem)
          )
        ) {
...
const searchChildsToHide = async function (parentNodesToCheck) {
  ...
  parentNodesToCheck.forEach((e) => {
    ...
    childs.forEach((child) => {
      elementsToSend.push({
        selector: filterest.getSelector(child),
  ...
  const requestOptions = {
    ...
    body: JSON.stringify({
      ...
      keywords: filterest.keywords,
    }),
  };

  await fetch(
    "https://localhost:44340/keywordfinder/similarElements",
    requestOptions
  )
    .then((response) => response.json())
    .then((data) => {
      data.forEach((elementSelector) => {
        var suggestedElement = htmlElementsToSent.find(
          (e) => filterest.getSelector(e) === elementSelector
        );
